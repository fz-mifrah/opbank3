...
describe('Operation e2e test', () => {
  ...
  const operationSample = { numOperation: 'drive', montant: 69939 };
  ...
  describe('new Operation page', () => {
    ...
    it('should create an instance of Operation', () => {
      ...
      cy.get(`[data-cy="date"]`).type('2022-05-09').should('have.value', '2022-05-09');

      cy.get(`[data-cy="typeOperatin"]`).select('TypeRecharge');

      cy.get(`[data-cy="etatOperation"]`).type('Handmade Peso drive').should('have.value', 'Handmade Peso drive');

      cy.get(`[data-cy="montant"]`).type('61378').should('have.value', '61378');
